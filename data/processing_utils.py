...
def random_crop(frames, crop_sz, return_crop_info=False):
    """
    :param frames: Input frame as tensor
    :param crop_sz: Output crop sz as (rows, cols)
    :param return_crop_info: Whether to return the co-ordinates of box used for cropping
    :return:
    """
    if not isinstance(crop_sz, (tuple, list)):
        crop_sz = (crop_sz, crop_sz)
    ...
    assert crop_sz[-2] <= shape[-2] and crop_sz[-1] <= shape[-1], 'Incorrect crop size'

    r1 = random.randint(0, shape[-2] - crop_sz[-2])
    c1 = random.randint(0, shape[-1] - crop_sz[-1])

    r2 = r1 + crop_sz[0].int().item()
    c2 = c1 + crop_sz[1].int().item()

    out = frames[:, r1:r2, c1:c2]

    if return_crop_info:
        return out, (r1, r2, c1, c2)
    else:
        return out
